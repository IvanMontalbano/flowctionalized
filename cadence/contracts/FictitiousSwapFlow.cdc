...
import FlowTokenFictitious from "./FlowTokenFictitious.cdc"

// Exchange pair between FlowToken and FlowTokenFictitious
// Token1: FlowToken
// Token2: FlowTokenFictitious
pub contract FictitiousSwapFlow: FungibleToken {
  ...
  // Controls FlowTokenFictitious vault
  access(contract) let token2Vault: @FlowTokenFictitious.Vault
  ...
  pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance {
    ...
    pub fun deposit(from: @FungibleToken.Vault) {
      let vault <- from as! @FictitiousSwapFlow.Vault
    ...
    destroy() {
      FictitiousSwapFlow.totalSupply = FictitiousSwapFlow.totalSupply - self.balance
    }
  }
  ...
  pub resource TokenBundle {
    ...
    pub var token2: @FlowTokenFictitious.Vault

    // initialize the vault bundle
    init(fromToken1: @FlowToken.Vault, fromToken2: @FlowTokenFictitious.Vault) {
    ...
    pub fun depositToken2(from: @FlowTokenFictitious.Vault) {
    ...
    pub fun withdrawToken2(): @FlowTokenFictitious.Vault {
      var vault <- FlowTokenFictitious.createEmptyVault() as! @FlowTokenFictitious.Vault
      vault <-> self.token2
      return <- vault
    }
  ...
  // createEmptyBundle
  //
  pub fun createEmptyTokenBundle(): @FictitiousSwapFlow.TokenBundle {
    return <- create TokenBundle(
      fromToken1: <- (FlowToken.createEmptyVault() as! @FlowToken.Vault),
      fromToken2: <- (FlowTokenFictitious.createEmptyVault() as! @FlowTokenFictitious.Vault)
    )
  }

  // createTokenBundle
  //
  pub fun createTokenBundle(fromToken1: @FlowToken.Vault, fromToken2: @FlowTokenFictitious.Vault): @FictitiousSwapFlow.TokenBundle {
  ...
  //
  access(contract) fun mintTokens(amount: UFix64): @FictitiousSwapFlow.Vault {
    pre {
      amount > 0.0: "Amount minted must be greater than zero"
    }
    FictitiousSwapFlow.totalSupply = FictitiousSwapFlow.totalSupply + amount
  ...
  //
  access(contract) fun burnTokens(from: @FictitiousSwapFlow.Vault) {
    let vault <- from as! @FictitiousSwapFlow.Vault
  ...
  pub resource SwapProxy {
    pub fun swapToken1ForToken2(from: @FlowToken.Vault): @FlowTokenFictitious.Vault {
      return <- FictitiousSwapFlow._swapToken1ForToken2(from: <-from)
    }

    pub fun swapToken2ForToken1(from: @FlowTokenFictitious.Vault): @FlowToken.Vault {
      return <- FictitiousSwapFlow._swapToken2ForToken1(from: <-from)
    }

    pub fun addLiquidity(from: @FictitiousSwapFlow.TokenBundle): @FictitiousSwapFlow.Vault {
      return <- FictitiousSwapFlow._addLiquidity(from: <-from)
    }

    pub fun removeLiquidity(from: @FictitiousSwapFlow.Vault): @FictitiousSwapFlow.TokenBundle {
      return <- FictitiousSwapFlow._removeLiquidity(from: <-from)
    }
  }

  pub resource Admin {
    pub fun freeze() {
      FictitiousSwapFlow.isFrozen = true
    }

    pub fun unfreeze() {
      FictitiousSwapFlow.isFrozen = false
    }

    pub fun setProxyOnly(proxyOnly: Bool) {
      FictitiousSwapFlow.account.load<Bool>(from: /storage/proxyOnly)
      FictitiousSwapFlow.account.save(proxyOnly, to: /storage/proxyOnly)
    }

    pub fun addInitialLiquidity(from: @FictitiousSwapFlow.TokenBundle): @FictitiousSwapFlow.Vault {
      pre {
        FictitiousSwapFlow.totalSupply == 0.0: "Pair already initialized"
      }
      ...
      FictitiousSwapFlow.token1Vault.deposit(from: <- token1Vault)
      FictitiousSwapFlow.token2Vault.deposit(from: <- token2Vault)

      destroy from

      // Create initial tokens
      return <- FictitiousSwapFlow.mintTokens(amount: 1.0)
    }

    pub fun updateFeePercentage(feePercentage: UFix64) {
      FictitiousSwapFlow.feePercentage = feePercentage

      emit FeeUpdated(feePercentage: feePercentage)
    }

    pub fun createSwapProxy(): @FictitiousSwapFlow.SwapProxy {
      return <- create FictitiousSwapFlow.SwapProxy()
    }
  }
  ...
  pub fun getPoolAmounts(): PoolAmounts {
    return PoolAmounts(token1Amount: FictitiousSwapFlow.token1Vault.balance, token2Amount: FictitiousSwapFlow.token2Vault.balance)
  }
  ...
  access(contract) fun _swapToken1ForToken2(from: @FlowToken.Vault): @FlowTokenFictitious.Vault {
    pre {
      !FictitiousSwapFlow.isFrozen: "FictitiousSwapFlow is frozen"
    ...
    return <- (self.token2Vault.withdraw(amount: token2Amount) as! @FlowTokenFictitious.Vault)
  }

  pub fun swapToken1ForToken2(from: @FlowToken.Vault): @FlowTokenFictitious.Vault {
    pre {
      !FictitiousSwapFlow.proxyOnly(): "FictitiousSwapFlow is proxyOnly"
    }
  ...
  access(contract) fun _swapToken2ForToken1(from: @FlowTokenFictitious.Vault): @FlowToken.Vault {
    pre {
      !FictitiousSwapFlow.isFrozen: "FictitiousSwapFlow is frozen"
      from.balance > 0.0: "Empty token vault"
    }
  ...
  pub fun donateLiquidity(from: @FictitiousSwapFlow.TokenBundle) {
    let token1Vault <- from.withdrawToken1()
    let token2Vault <- from.withdrawToken2()
  ...
  access(contract) fun _addLiquidity(from: @FictitiousSwapFlow.TokenBundle): @FictitiousSwapFlow.Vault {
    pre {
    ...
    let token1Percentage: UFix64 = (token1Vault.balance * 10000.0) / FictitiousSwapFlow.token1Vault.balance
    let token2Percentage: UFix64 = (token2Vault.balance * 10000.0) / FictitiousSwapFlow.token2Vault.balance
    ...
    let liquidityTokenVault <- FictitiousSwapFlow.mintTokens(amount: (FictitiousSwapFlow.totalSupply * liquidityPercentage) / 10000.0)
  ...
  access(contract) fun _removeLiquidity(from: @FictitiousSwapFlow.Vault): @FictitiousSwapFlow.TokenBundle {
    pre {
      from.balance > 0.0: "Empty liquidity token vault"
      from.balance < FictitiousSwapFlow.totalSupply: "Cannot remove all liquidity"
    }

    // shift decimal 4 places to avoid truncation error
    let liquidityPercentage = (from.balance * 10000.0) / FictitiousSwapFlow.totalSupply

    assert(liquidityPercentage > 0.0, message: "Liquidity too small")

    // Burn liquidity tokens and withdraw
    FictitiousSwapFlow.burnTokens(from: <- from)

    let token1Vault <- FictitiousSwapFlow.token1Vault.withdraw(amount: (FictitiousSwapFlow.token1Vault.balance * liquidityPercentage) / 10000.0) as! @FlowToken.Vault
    let token2Vault <- FictitiousSwapFlow.token2Vault.withdraw(amount: (FictitiousSwapFlow.token2Vault.balance * liquidityPercentage) / 10000.0) as! @FlowTokenFictitious.Vault

    let tokenBundle <- FictitiousSwapFlow.createTokenBundle(fromToken1: <- token1Vault, fromToken2: <- token2Vault)
    return <- tokenBundle
  }
  ...
  init() {
    ...
    // Setup internal FlowTokenFictitious vault
    self.token2Vault <- FlowTokenFictitious.createEmptyVault() as! @FlowTokenFictitious.Vault
